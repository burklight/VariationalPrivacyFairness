...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        img, target, hidden = self.data[index], self.targets[index], self.hidden[index]
        img, target, hidden = torchvision.transforms.functional.to_pil_image(img), int(target), int(hidden)
        if self.transform is not None:
            img = self.transform(img)
        return img, target, hidden
...
def get_mnist():
    ...
    data_n /= 255.0 
    trainset = CustomDataset(data_n,trainset.targets,data_hidden,trainset.transform)
    N_tst = len(testset)
    ...
    data_n /= 255.0 
    testset = CustomDataset(data_n,testset.targets,data_hidden,testset.transform)
...
def get_adult():
    ...
    data_train = data_train.apply(lambda v: v.str.strip() if v.dtype == "object" else v)
    data_test = data_test.apply(lambda v: v.str.strip() if v.dtype == "object" else v)
    ...
    trainset = CustomDataset(X_train, T_train, S_train)
    testset = CustomDataset(X_test, T_test, S_test)
