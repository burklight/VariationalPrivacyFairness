...
def distances(X, v, alpha, N, k):
    ...
    for i in range(N):
        for j in range(k):
            ...
            if np.isnan(dists[i,j]):
                print(np.sum(X), X.shape, X[i,:].shape, X[i-1:i+1,:].shape)
                print(X[i,:])
                return
    return dists
...
def LFR(params, data_sensitive, data_nonsensitive, y_sensitive, 
    ...
    dists_sensitive = distances(data_sensitive, v, alpha1, Ns, k)
    dists_nonsensitive = distances(data_nonsensitive, v, alpha0, Nns, k)
