...
class Encoder(torch.nn.Module): 
    ...
    def forward(self, x, s): 
        ...
        z_mu_logvar = self.f(torch.cat((x,s.view(-1,self.sensitive_dim)),1))
...
class Decoder(torch.nn.Module): 
    ...
    def forward(self, z, s): 
        ...
        return self.f(torch.cat((z,s.view(-1,self.sensitive_dim)),1)) 


class VFAE(torch.nn.Module): 
    ...
    def get_mmd(self, z, s, device):

        D = 500
        gamma = 1
        z_0 = z[s==0]
        z_1 = z[s==1]
        W_0, b_0 = torch.randn((z_0.shape[1],D)), -2 * math.pi * torch.rand(D) + 2 * math.pi
        W_1, b_1 = torch.randn((z_1.shape[1],D)), -2 * math.pi * torch.rand(D) + 2 * math.pi
        W_0, b_0, W_1, b_1 = W_0.to(device), b_0.to(device), W_1.to(device), b_1.to(device)
        psi_0 = torch.mean(math.sqrt(2/D) * torch.cos(math.sqrt(2/gamma) * torch.mm(z_0, W_0) + b_0))
        psi_1 = torch.mean(math.sqrt(2/D) * torch.cos(math.sqrt(2/gamma) * torch.mm(z_1, W_1) + b_1))

        return len(z) * torch.pow(torch.norm(psi_0 - psi_1), 2)
    ...
    def train_step(self, batch_size, dataloader, optimizer, verbose=True):
        ...
        for x_in, x_out, s in progressbar(dataloader):
